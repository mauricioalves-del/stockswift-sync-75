-- 1. Motor: processar qualquer data com movimentação (sem bloquear datas passadas)
...
REVOKE EXECUTE ON FUNCTION public.processar_fefo(date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.processar_fefo(date) TO authenticated, service_role;

-- 2. Gatilho: reprocessar todos os dias com movimentação pendente de checagem
...
BEGIN
  FOR r IN
    ...
    FROM public.movimentacoes_diarias m
    WHERE (m.data AT TIME ZONE 'America/Sao_Paulo')::date
...
-- 3. Recalcular o que já está importado
DO $$
DECLARE r record;
BEGIN
  FOR r IN SELECT DISTINCT (m.data AT TIME ZONE 'America/Sao_Paulo')::date AS d
           FROM public.movimentacoes_diarias m LOOP
    PERFORM public.processar_fefo(r.d);
  END LOOP;
END $$;