    ...
    SELECT f.*,
      CASE
        ...
        WHEN EXISTS (SELECT 1 FROM public.excecoes_fefo e
                     WHERE upper(btrim(coalesce(e.id_produto,''))) = upper(btrim(coalesce(f.id_produto,'')))
...
BEGIN
  FOR r IN
    SELECT DISTINCT c.data AS d
    FROM public.checagens_fefo c
    WHERE c.data >= (now() AT TIME ZONE 'America/Sao_Paulo')::date - 45
  LOOP
...
DROP TRIGGER IF EXISTS trg_reprocessar_fefo_excecoes ON public.excecoes_fefo;
CREATE TRIGGER trg_reprocessar_fefo_excecoes
AFTER INSERT OR UPDATE OR DELETE ON public.excecoes_fefo
FOR EACH STATEMENT EXECUTE FUNCTION public.reprocessar_fefo_dias_afetados();