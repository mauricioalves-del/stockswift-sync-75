-- 1. Limpar histórico
DELETE FROM public.checagens_fefo;
DELETE FROM public.movimentacoes_diarias;

-- 2. Desagendar o job diário
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    PERFORM cron.unschedule(jobid) FROM cron.job WHERE command ILIKE '%processar_fefo%';
  END IF;
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

-- 3. Motor: ignorar datas anteriores a hoje (America/Sao_Paulo)
...
-- 4. Rodar o motor sempre que a tabela de movimentações for atualizada
...
BEGIN
  FOR r IN
    SELECT DISTINCT (m.data AT TIME ZONE 'America/Sao_Paulo')::date AS d
    FROM public.movimentacoes_diarias m
    WHERE (m.data AT TIME ZONE 'America/Sao_Paulo')::date >= hoje
  LOOP
...
DROP TRIGGER IF EXISTS trg_fefo_apos_movimentacao ON public.movimentacoes_diarias;
CREATE TRIGGER trg_fefo_apos_movimentacao
AFTER INSERT OR UPDATE OR DELETE ON public.movimentacoes_diarias
FOR EACH STATEMENT EXECUTE FUNCTION public.trigger_fefo_apos_movimentacao();

REVOKE ALL ON FUNCTION public.trigger_fefo_apos_movimentacao() FROM PUBLIC, anon;