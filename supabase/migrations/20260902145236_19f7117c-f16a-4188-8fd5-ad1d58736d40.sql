...
BEGIN
  ...
  IF d <> hoje THEN
    RAISE EXCEPTION 'Somente o dia atual pode ser reprocessado; dias encerrados permanecem imutáveis.';
  END IF;
  ...
  SELECT
    d, f.id_produto, f.descricao, f.desc_movimento, f.desc_almox, f.dest_txt, f.doc,
    f.id_lote, f.qtd, f.val_mov,
    f.status_final = '⚠️ QUEBRA DE FEFO', f.status_final,
    f.lote_mais_antigo, f.qtd_lote_mais_antigo, f.validade_mais_antiga
  FROM (
    SELECT base.*,
      CASE
        ...
        WHEN EXISTS (
          SELECT 1 FROM public.excecoes_fefo ex
    ...
    FROM (
      SELECT
        m.id_produto, m.descricao, m.doc, m.desc_movimento, m.desc_almox, m.qtd, m.id_lote,
        ...
        ((lower(btrim(split_part(m.desc_movimento, '->', 1))) LIKE '%fabrica%'
          ...
          AND origem.codigo IS NOT NULL AND origem.codigo = origem_mov.codigo) AS eh_origem,
      ...
      LEFT JOIN LATERAL (
        ...
        WHERE ma.ativo AND lower(btrim(ma.nome)) = lower(btrim(m.desc_almox)) LIMIT 1
      ) origem ON true
      LEFT JOIN LATERAL (
        SELECT ma.codigo FROM public.mapa_almoxarifados ma
        WHERE ma.ativo AND lower(btrim(ma.nome)) = lower(btrim(split_part(m.desc_movimento, '->', 1))) LIMIT 1
      ) origem_mov ON true
      LEFT JOIN LATERAL (
        SELECT min(e.data_validade) AS validade FROM public.estoque_sistemico e
      ...
      LEFT JOIN LATERAL (
        SELECT count(*) FILTER (WHERE e.quantidade > 0) AS total_com_saldo,
               count(*) FILTER (WHERE e.quantidade > 0 AND e.data_validade IS NOT NULL) AS total_com_validade
        FROM public.estoque_sistemico e
        WHERE upper(btrim(e.id_produto)) = upper(btrim(m.id_produto)) AND e.origem = origem.codigo
      ) saldos ON true
      LEFT JOIN LATERAL (
        SELECT e.lote, e.quantidade, e.data_validade FROM public.estoque_sistemico e
        WHERE upper(btrim(e.id_produto)) = upper(btrim(m.id_produto))
          AND e.origem = origem.codigo AND e.quantidade > 0 AND e.data_validade IS NOT NULL
        ORDER BY e.data_validade ASC, upper(btrim(e.lote)) ASC, e.id ASC LIMIT 1
      ) antigo ON true
      WHERE (m.data AT TIME ZONE 'America/Sao_Paulo')::date = d AND m.desc_movimento LIKE '%->%'
    ) base
  ) f;

  INSERT INTO public.checagens_fefo_lotes_snapshot (
    checagem_id, estoque_sistemico_id, id_produto, origem, lote, quantidade, data_validade, data_importacao
  )
  SELECT c.id, e.id, e.id_produto, e.origem, e.lote, e.quantidade, e.data_validade, e.data_importacao
  FROM public.checagens_fefo c
  JOIN public.mapa_almoxarifados ma
    ON ma.ativo AND lower(btrim(ma.nome)) = lower(btrim(c.desc_almox))
  JOIN public.estoque_sistemico e
    ON upper(btrim(e.id_produto)) = upper(btrim(c.id_produto))
   AND e.origem = ma.codigo AND e.quantidade > 0
  WHERE c.data = d;
...
REVOKE EXECUTE ON FUNCTION public.processar_fefo(date) FROM authenticated;
