// Edge Function: Farol de Shelf (envio semanal, toda segunda-feira 09:30).
// Replica a lógica da tela /shelf-life/farol: risco de perda por faixa de
// validade (Urgente 0-30, Perigo 31-60, Atenção 61-90, Vencido), com custo
// em risco por grupo/almoxarifado. Anexa um painel HTML interativo (mesmo
// motor de filtro cruzado usado nos farois de Dispersão e FEFO) e inclui um
// link direto para a tela no sistema.
//
...
const FINALIDADE = "Farol de Shelf Semanal - PA";
const EMPRESA_ALVO = "Matriz Para";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const cronSecretEsperado = await getCfg(admin, "farol_shelf_pa_cron_secret");
    ...
    // ==== Estoque em risco (mesma fonte da tela: estoque_sistemico, saldo > 0) ====
    const [estoqueRes, gruposRes, familiasRes] = await Promise.all([
      admin.from("estoque_sistemico")
        .select("id_produto, descricao, lote, unidade, quantidade, custo_unitario, id_local, origem, data_validade")
        .gt("quantidade", 0),
      admin.from("grupo_produtos").select("codigo_produto, grupo"),
      admin.from("familias").select("codigo_produto, familia"),
    ]);
    if (estoqueRes.error) throw estoqueRes.error;

    const norm = (s: unknown) => String(s ?? "").trim().toUpperCase();
    const gMap = new Map<string, string>((gruposRes.data ?? []).map((g: any) => [norm(g.codigo_produto), g.grupo]));
    const fMap = new Map<string, string>((familiasRes.data ?? []).map((f: any) => [norm(f.codigo_produto), f.familia]));

    const STATUS_POR_FAIXA: Record<string, string> = { VENCIDO: "Vencido", "30": "Urgente", "60": "Perigo", "90": "Atenção" };

    const linhas = (estoqueRes.data ?? [])
      .map((r: any) => {
        ...
        return {
          ...
          grupo: gMap.get(norm(r.id_produto)) ?? "Sem grupo",
          familia: fMap.get(norm(r.id_produto)) ?? "Sem família",
          quantidade: qtd,
          custo_unitario: custo,
          valor: qtd * custo,
          data_validade: r.data_validade ?? null,
          status: STATUS_POR_FAIXA[faixa],
        };
      })
    ...
    // ==== KPIs por status ====
    const ORDEM = ["Urgente", "Perigo", "Atenção", "Vencido"];
    const totais = new Map<string, number>();
    for (const r of linhas) totais.set(r.status, (totais.get(r.status) ?? 0) + r.valor);
    const totalGeral = linhas.reduce((s: number, r: any) => s + r.valor, 0);

    const CORES: Record<string, string> = { Urgente: "#F1704B", Perigo: "#F2C14E", "Atenção": "#2CC5A8", Vencido: "#7B3B2E" };

    const kpisHtml = ORDEM.map((st) => `
      <div style="display:inline-block;width:23%;min-width:150px;margin:0 1% 8px 0;vertical-align:top;padding:10px 12px;border:1px solid #e5e7eb;border-radius:8px;border-left:4px solid ${CORES[st]}">
        <div style="font-size:10px;text-transform:uppercase;color:#6b7280">${esc(st)}</div>
        <div style="font-size:16px;font-weight:700;color:#111827">R$ ${formatBRL(totais.get(st) ?? 0)}</div>
      </div>`).join("");
...
<h2 style="margin:0 0 4px">Farol de Shelf — Matriz Pará — ${esc(agoraFmt)}</h2>
<p style="font-size:12px;color:#6b7280;margin:0 0 16px">Risco total em ${linhas.length} lote(s): <strong>R$ ${formatBRL(totalGeral)}</strong></p>
${kpisHtml}
<p style="margin:20px 0">
  <a href="${APP_URL}" style="background:#111827;color:#ffffff;text-decoration:none;padding:10px 18px;border-radius:8px;font-size:13px;font-weight:600;display:inline-block">Abrir Farol de Shelf na plataforma →</a>
</p>
<hr style="margin:24px 0;border:none;border-top:1px solid #e5e7eb" />
<p style="font-size:11px;color:#6b7280">Em anexo, o painel interativo (BI ativo) com o detalhamento por almoxarifado, grupo e produto — clique nas barras/fatias/linhas para filtrar. Enviado automaticamente toda segunda-feira pelo Controle Operacional.</p>
</body></html>`;
    ...
    const htmlInterativo = montarHtmlInterativo({
      titulo: "Farol de Shelf — Matriz Pará — Risco de Perda por Validade",
      subtitulo: "Custo em risco por faixa de validade, almoxarifado, grupo e produto (operação Pará)",
      linhas: linhasBI,
      dimensoes: [
      ...
      medida: { chave: "valor", rotulo: "Custo em risco", formato: "brl" },
      ...
      colunas: [
        ...
        { chave: "valor", rotulo: "Custo em risco", formato: "brl" },
        { chave: "data_validade", rotulo: "Validade" },
        { chave: "status", rotulo: "Status" },
      ],
    ...
    const raw = buildRawEmail({
      ...
      to: toList,
      replyTo: REPLY_TO,
      subject: `Farol de Shelf — Matriz Pará — ${agoraFmt.slice(0, 10)} (R$ ${formatBRL(totalGeral)} em risco)`,
      html,
      attachment: {
        filename: `${slug("Farol de Shelf Matriz Para")}_${hojeISO}.html`,
    ...
    await admin.from("audit_logs").insert({
      usuario: null,
      acao: r.ok ? "FAROL_SHELF_ENVIADO" : "FAROL_SHELF_FALHA",
      entidade: "estoque_sistemico",
      payload: {
        gerado_em: hojeISO,
        destinatarios: toList,
        qtd_lotes: linhas.length,
    ...
    return json({ ok: true, gerado_em: hojeISO, qtd_lotes: linhas.length, total_risco: totalGeral, destinatarios: toList, gmail_id: r.id ?? null });
  } catch (err: any) {
    console.error("farol-shelf-semanal", err);
