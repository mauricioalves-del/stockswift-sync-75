...
const FINALIDADE = "Farol Dispersão Diário - PA";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const cronSecretEsperado = await getCfg(admin, "farol_dispersao_pa_cron_secret");
...
<h2 style="margin:0 0 8px">Farol de Dispersão de Lote — Matriz Pará — ${esc(dataAlvoFmt)}</h2>
${corpo}
...
<p style="font-size:11px;color:#6b7280">Enviado automaticamente todo dia útil pelo Controle Operacional.</p>
    ...
    const htmlInterativo = montarHtmlInterativo({
      titulo: "Dispersão de Lote — Produção — Matriz Pará",
      subtitulo: "Ficha Técnica × Consumo real por Ordem de Produção (operação Pará)",
      linhas: linhasBI,
      dimensoes: [
    ...
    const raw = buildRawEmail({
      ...
      to: toList,
      replyTo: REPLY_TO,
      subject: `Farol de Dispersão de Lote — Matriz Pará — ${dataAlvoFmt}${itens.length ? ` (${itens.length} desvio(s))` : ""}`,
      html,
      attachment: {
        filename: `${slug("Dispersao de Lote Producao Matriz Para")}_${dataAlvo}.html`,
