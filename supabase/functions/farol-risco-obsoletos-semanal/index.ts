// Edge Function: Farol de Risco - Movimentados (envio semanal, toda segunda-feira 09:30).
// Replica a lógica da tela /suprimentos/risco-obsoletos: itens em estoque sem
// movimentação sistêmica (entradas, transferências, consumo em produção ou
// conclusão de OP), classificados por faixa de dias sem movimento (30-60,
// 61-90, +90, sem registro) e por empresa (Filial SP - Fábrica / Matriz Pará).
// Anexa um painel HTML interativo (mesmo motor de filtro cruzado usado nos
// farois de Dispersão, FEFO e Shelf) e inclui um link direto para a tela no sistema.
//
...
const FINALIDADE = "Farol de Risco - Movimentados";
const APP_URL = "https://stockswift-sync-75.lovable.app/suprimentos/risco-obsoletos";
...
function buildRawEmail(opts: {
  ...
  const boundary = "----=_farol_shelf_" + crypto.randomUUID().replace(/-/g, "");
...
// Faixa de risco (mesma lógica de src/lib/shelf-life.ts::faixaDeRisco).
function faixaDeRisco(validade: string | null, hojeISO: string): "VENCIDO" | "30" | "60" | "90" | null {
  if (!validade) return null; // PENDENTE — fora do farol
  const hoje = new Date(`${hojeISO}T00:00:00`);
  const dv = new Date(`${String(validade).slice(0, 10)}T00:00:00`);
  const dias = Math.round((dv.getTime() - hoje.getTime()) / 86_400_000);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const cronSecretEsperado = await getCfg(admin, "farol_risco_obsoletos_cron_secret");
    ...
    const hojeISO = `${sp.getFullYear()}-${String(sp.getMonth() + 1).padStart(2, "0")}-${String(sp.getDate()).padStart(2, "0")}`;
    ...
    // ==== Itens em risco (mesma fonte da tela: view v_risco_obsoletos) ====
    const linhasRaw: any[] = [];
    const pageSize = 1000;
    for (let from = 0; ; from += pageSize) {
      const { data, error } = await admin
        .from("v_risco_obsoletos")
        .select("id_produto, descricao, almoxarifado, lote, saldo, valor, empresa, ultima_mov, dias_sem_mov, faixa")
    ...
    const STATUS_LABEL: Record<string, string> = {
      "30-60": "30 a 60 dias", "61-90": "61 a 90 dias", "+90": "Mais de 90 dias", sem_registro: "Sem registro",
    };

    const linhas = linhasRaw.map((r: any) => ({
      ...
      ultima_mov: r.ultima_mov ?? null,
      dias_sem_mov: r.dias_sem_mov ?? null,
      status: STATUS_LABEL[r.faixa] ?? r.faixa,
      faixa: r.faixa,
    }));
    ...
    for (const r of linhas) {
      totais.set(r.faixa, (totais.get(r.faixa) ?? 0) + r.valor);
      contagens.set(r.faixa, (contagens.get(r.faixa) ?? 0) + 1);
    }
    const totalGeral = linhas.reduce((s: number, r: any) => s + r.valor, 0);

    const CORES: Record<string, string> = { "30-60": "#F2C14E", "61-90": "#F1704B", "+90": "#B23A2E", sem_registro: "#7B3B2E" };

    const kpisHtml = ORDEM.map((fx) => `
      <div style="display:inline-block;width:23%;min-width:150px;margin:0 1% 8px 0;vertical-align:top;padding:10px 12px;border:1px solid #e5e7eb;border-radius:8px;border-left:4px solid ${CORES[fx]}">
        <div style="font-size:10px;text-transform:uppercase;color:#6b7280">${esc(STATUS_LABEL[fx])}</div>
        <div style="font-size:16px;font-weight:700;color:#111827">R$ ${formatBRL(totais.get(fx) ?? 0)}</div>
        <div style="font-size:11px;color:#6b7280">${contagens.get(fx) ?? 0} item(ns)</div>
      </div>`).join("");
...
<h2 style="margin:0 0 4px">Farol de Risco - Movimentados — ${esc(agoraFmt)}</h2>
<p style="font-size:12px;color:#6b7280;margin:0 0 16px">Risco total em ${linhas.length} item(ns) sem movimentação: <strong>R$ ${formatBRL(totalGeral)}</strong></p>
${kpisHtml}
<p style="margin:20px 0">
  <a href="${APP_URL}" style="background:#111827;color:#ffffff;text-decoration:none;padding:10px 18px;border-radius:8px;font-size:13px;font-weight:600;display:inline-block">Abrir Risco Obsoletos na plataforma →</a>
</p>
<hr style="margin:24px 0;border:none;border-top:1px solid #e5e7eb" />
<p style="font-size:11px;color:#6b7280">Em anexo, o painel interativo (BI ativo) com o detalhamento por faixa, empresa, almoxarifado e produto — clique nas barras/fatias/linhas para filtrar. Enviado automaticamente toda segunda-feira pelo Controle Operacional.</p>
</body></html>`;

    const linhasBI = linhas.map((r: any) => ({
      sku: r.sku,
    ...
    const htmlInterativo = montarHtmlInterativo({
      titulo: "Farol de Risco - Movimentados",
      subtitulo: "Itens em estoque sem movimentação sistêmica, por faixa de dias, empresa, almoxarifado e produto",
      linhas: linhasBI,
      dimensoes: [
        { chave: "status", rotulo: "Faixa" },
        { chave: "empresa", rotulo: "Empresa" },
        { chave: "almoxarifado", rotulo: "Almoxarifado" },
        { chave: "sku", rotulo: "Produto", chaveRotulo: "descricao" },
      ],
      medida: { chave: "valor", rotulo: "Valor em risco", formato: "brl" },
      medidaSecundaria: { chave: "quantidade", rotulo: "Quantidade", formato: "num" },
      colunas: [
        { chave: "sku", rotulo: "Código" },
        { chave: "descricao", rotulo: "Produto" },
        { chave: "lote", rotulo: "Lote" },
        { chave: "almoxarifado", rotulo: "Almoxarifado" },
        { chave: "empresa", rotulo: "Empresa" },
        { chave: "quantidade", rotulo: "Qtd", formato: "num" },
        { chave: "valor", rotulo: "Valor em risco", formato: "brl" },
        { chave: "dias_sem_mov", rotulo: "Dias sem mov.", formato: "num" },
        { chave: "status", rotulo: "Faixa" },
      ],
      filtrosAtivos: [{ label: "Gerado em", valor: agoraFmt }],
    });
    ...
    const raw = buildRawEmail({
      ...
      to: toList,
      replyTo: REPLY_TO,
      subject: `Farol de Risco - Movimentados — ${agoraFmt.slice(0, 10)} (R$ ${formatBRL(totalGeral)} em risco)`,
      html,
      attachment: {
        filename: `${slug("Farol de Risco Movimentados")}_${hojeISO}.html`,
    ...
    await admin.from("audit_logs").insert({
      usuario: null,
      acao: r.ok ? "FAROL_RISCO_OBSOLETOS_ENVIADO" : "FAROL_RISCO_OBSOLETOS_FALHA",
      entidade: "v_risco_obsoletos",
      payload: {
        gerado_em: hojeISO,
        destinatarios: toList,
        qtd_itens: linhas.length,
        total_risco: totalGeral,
    ...
    return json({ ok: true, gerado_em: hojeISO, qtd_itens: linhas.length, total_risco: totalGeral, destinatarios: toList, gmail_id: r.id ?? null });
  } catch (err: any) {
    console.error("farol-risco-obsoletos-semanal", err);
