...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const cronSecretEsperado = await getCfg(admin, "farol_shelf_cron_secret");
...
<h2 style="margin:0 0 4px">Farol de Shelf — ${esc(agoraFmt)}</h2>
...
<p style="font-size:11px;color:#6b7280">Em anexo, o painel interativo (BI ativo) com o detalhamento por almoxarifado, grupo e produto — clique nas barras/fatias/linhas para filtrar. Enviado automaticamente toda segunda-feira pelo Controle Operacional.</p>
    ...
    const htmlInterativo = montarHtmlInterativo({
      titulo: "Farol de Shelf — Risco de Perda por Validade",
      subtitulo: "Custo em risco por faixa de validade, almoxarifado, grupo e produto",
      linhas: linhasBI,
      dimensoes: [
    ...
    const raw = buildRawEmail({
      ...
      to: toList,
      replyTo: REPLY_TO,
      subject: `Farol de Shelf — ${agoraFmt.slice(0, 10)} (R$ ${formatBRL(totalGeral)} em risco)`,
      html,
      attachment: {
        filename: `${slug("Farol de Shelf")}_${hojeISO}.html`,
